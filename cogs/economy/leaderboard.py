...
class LeaderboardCommand(commands.Cog):
	...
	async def leaderboard(self, ctx: commands.Context, lb: str = "balance"):
		...
		sorted_balances = sorted(user_list.items(), key=lambda x: int(x[1]), reverse=True)

		embed = discord.Embed(title=f"Leaderboard for {lb}", description="These are the top 10 users.",
		...
		for x in range(10):
			...
			embed.add_field(name=f"#{x + 1}", value=f"<@{user_id}> with {balance_message}", inline=False)
